...
class WindowsSMBHelper(object):
    _ACCESS_RIGHT_FULL = "Full"
    _ACCESS_RIGHT_CHANGE = "Change"
    _ACCESS_RIGHT_READ = "Read"

    _ACCESS_RIGHT_MAP = {
        constants.ACCESS_LEVEL_RW: _ACCESS_RIGHT_CHANGE,
        constants.ACCESS_LEVEL_RO: _ACCESS_RIGHT_READ}
    ...
    def remove_export(self, server, share_name):
        if self._share_exists(server, share_name):
            ...
            LOG.debug(_LI("Skipping removing export %s as "
                          "it does not exist."),
                      share_name)
    ...
    def allow_access(self, server, share_name, access_type, access_level,
        ...
        self._unblock_access(server, share_name, access_to)
        self._set_access(server, share_name, access_level, access_to)

    def _set_access(self, server, share_name, access_level, access_to):
        access_right = self._ACCESS_RIGHT_MAP[access_level]
    ...
    def deny_access(self, server, share_name, access, force=False):
        ...
        self._block_access(server, share_name, access_to)

    def _block_access(self, server, share_name, access_to):
        cmd = ['Block-SmbShareAccess', '-Name', share_name,
               '-AccountName', access_to, '-Force']
        self._remote_exec(server, cmd)

    def _unblock_access(self, server, share_name, access_to):
        cmd = ['Unblock-SmbShareAccess', '-Name', share_name,
               '-AccountName', access_to, '-Force']
        self._remote_exec(server, cmd)
    ...
    def get_share_path_by_export_location(self, server, export_location):
        ...
        return self._get_share_path_by_name(share_name)
