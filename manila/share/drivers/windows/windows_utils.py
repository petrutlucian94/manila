...
class WindowsUtils(object):
    ...
    def add_access_path(self, server, mount_path, disk_number,
        ...
        cmd = ["Add-PartitionAccessPath", "-DiskNumber", disk_number,
               ...
               "-AccessPath", self._quote_string(mount_path)]
        self._remote_exec(server, cmd)
    ...
    def get_disk_size_by_path(self, server, mount_path):
        cmd = ["fsutil", "volume", "diskfree",
               self._quote_string(mount_path)]
        (out, err) = self._remote_exec(server, cmd)

        total_bytes = long(self._fsutil_total_space_regex.findall(out)[1])
        return total_bytes
    ...
    def ensure_directory_exists(self, server, path):
        cmd = ["New-Item", "-ItemType", "Directory",
               "-Force", "-Path", self._quote_string(path)]
        self._remote_exec(server, cmd)
        ...
        if self.path_exists(server, path):
            if is_junction:
                cmd = ('[System.IO.Directory]::Delete('
                       '%(path)s, %(recurse)d)'
                       % dict(path=self._quote_string(path),
                              recurse=recurse))
            else:
                cmd = ["Remove-Item", "-Confirm:$false",
                       "-Path", self._quote_string(path)]
                if force:
                    cmd += ['-Force']
                if recurse:
                    cmd += ['-Recurse']
    ...
    def set_win_reg_value(self, server, path, key, value):
        cmd = ['Set-ItemProperty', '-Path', self._quote_string(path),
               '-Name', key, '-Value', value]
        self._remote_exec(server, cmd)

    def get_win_reg_value(self, server, path, name=None):
        cmd = "Get-ItemProperty -Path %s" % self._quote_string(path)
        if name:
