...
class WindowsSMBDriverTestCase(test.TestCase):
    ...
    def setUp(self, mock_sv_instance_mgr, mock_smb_helper_cls,
        ...
        self._drv = windows.WindowsSMBDriver(configuration=self._fake_conf)
    ...
    @mock.patch.object(windows.WindowsSMBDriver, '_get_disk_number')
    ...
    @mock.patch.object(windows.WindowsSMBDriver,
                       '_ensure_disk_online_and_writable')
    @mock.patch.object(windows.WindowsSMBDriver, '_get_disk_number')
    @mock.patch.object(windows.WindowsSMBDriver, '_get_mount_path')
    @mock.patch.object(windows.WindowsSMBDriver, '_is_device_mounted')
    ...
    @mock.patch.object(windows.WindowsSMBDriver,
                       '_ensure_disk_online_and_writable')
    def test_resize_filesystem(self, mock_ensure_disk, mock_get_disk_number):
        mock_get_disk_number.return_value = mock.sentinel.disk_number
        self._windows_utils.get_partition_maximum_size.return_value = (
            mock.sentinel.size)
    ...
    def test_get_mounted_share_size(self):
        fake_size_gb = 10
        self._windows_utils.get_disk_size_by_path.return_value = (
            fake_size_gb << 30)
